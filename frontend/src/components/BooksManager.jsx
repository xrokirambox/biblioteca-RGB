...
import { X, Plus, Save, Trash2, Edit3, Loader2, BookOpen, Check } from "lucide-react";
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4" data-testid="book-form">
      ...
      <div className="sm:col-span-2">
        ...
        <input
          ...
          value={form.cover}
          onChange={handle("cover")}
          placeholder="https://..."
          className="w-full bg-black/40 border border-[#c9a227]/20 rounded-sm px-3 py-2.5 text-sm font-dm-sans text-white focus:outline-none focus:border-[#c9a227] focus:ring-1 focus:ring-[#c9a227]"
        />
      </div>
      <div className="sm:col-span-2">
        <label className="block font-dm-sans text-[10px] tracking-widest uppercase text-[#c9a227]/80 mb-1.5">Enlace al libro (Google Drive, PDF, etc.)</label>
        <input
...
export const BooksManager = ({ open, onClose }) => {
  ...
  const handleCreate = async (form) => {
    try {
      ...
      toast.error(e.response?.data?.detail || "No se pudo crear el libro");
  ...
  return createPortal(
    <div
      ...
      onClick={onClose}
      data-testid="books-manager-overlay"
    >
      <div
        className="w-full max-w-5xl bg-[#051a09] border border-[#c9a227]/30 rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[92vh] animate-scale-in"
        onClick={(e) => e.stopPropagation()}
        data-testid="books-manager-modal"
      >
        <div className="p-5 sm:p-6 border-b border-[#c9a227]/20 bg-[#020b04]/60 flex items-center gap-4">
          ...
          <button
            onClick={onClose}
            data-testid="books-manager-close"
                    ...
                    >
                      ...
                      <div className="p-4 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto flex items-center gap-2">
                          <button
                            onClick={() => { setEditing(b); setMode("edit"); }}
                          ...
                          <button
                            ...
                            className="p-2 rounded-sm border border-red-900/50 text-red-400/80 hover:bg-red-950/50"
                            title="Eliminar"
                          >
