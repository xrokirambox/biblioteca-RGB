...
import { X, Plus, Save, Trash2, Edit3, Loader2, BookOpen } from "lucide-react";
...
const BookForm = ({ initial, onSubmit, onCancel, submitting }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4" data-testid="book-form">
      ...
      <div className="sm:col-span-2">
        <label className="block font-dm-sans text-[10px] tracking-widest uppercase text-[#c9a227]/80 mb-1.5">Enlace al libro (Drive, PDF, etc.)</label>
...
export const BooksManager = ({ open, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[55] bg-[#020b04]/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in"
      onClick={onClose} data-testid="books-manager-overlay">
      <div className="w-full max-w-5xl bg-[#051a09] border border-[#c9a227]/30 rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[92vh] animate-scale-in"
        onClick={(e) => e.stopPropagation()} data-testid="books-manager-modal">
        <div className="p-5 sm:p-6 border-b border-[#c9a227]/20 bg-[#020b04]/60 flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-[#c9a227]/10 border border-[#c9a227]/30 flex items-center justify-center">
            <BookOpen className="w-4 h-4 text-[#c9a227]" />
          </div>
          <div className="flex-1">
            <div className="font-cinzel text-xl text-[#f4f1e1]">Gestión de Libros</div>
            <div className="font-dm-sans text-[10px] tracking-[0.25em] uppercase text-[#c9a227]">Crear · Editar · Eliminar</div>
          </div>
          {mode === "list" && (
          ...
          <button onClick={onClose} data-testid="books-manager-close"
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          {mode === "create" && (
            <div className="animate-fade-in">
              <div className="font-cinzel text-lg text-[#c9a227] mb-5">Nuevo libro</div>
              <BookForm onSubmit={handleCreate} onCancel={() => setMode("list")} submitting={busy} />
            </div>
          )}
          {mode === "edit" && editing && (
            <div className="animate-fade-in">
              <div className="font-cinzel text-lg text-[#c9a227] mb-5">Editar libro</div>
            ...
            <div className="animate-fade-in">
              {books.length === 0 ? (
                <div className="text-center py-16 font-cormorant text-xl text-[#a3b3a6]">
                  Aún no hay libros. Crea el primero con el botón “Nuevo”.
                </div>
              ) : (
                    ...
                    <div key={b.id} data-testid={`admin-book-${b.id}`} className="glass rounded-lg overflow-hidden flex flex-col">
                      <div className="relative aspect-[3/4] overflow-hidden bg-black/40">
                        {b.cover ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-[#c9a227]/40">
                            <BookOpen className="w-10 h-10" />
                          </div>
                        )}
                        <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-[#020b04]/80 border border-[#c9a227]/30 font-dm-sans text-[9px] tracking-widest uppercase text-[#c9a227]">
                          {b.category}
                        </div>
                      </div>
                      <div className="p-4 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto flex items-center gap-2">
                          <button onClick={() => { setEditing(b); setMode("edit"); }}
                            data-testid={`admin-book-edit-${b.id}`}
                          ...
                          <button onClick={() => handleDelete(b)}
                            data-testid={`admin-book-delete-${b.id}`}
